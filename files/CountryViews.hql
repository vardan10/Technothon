...
load data local inpath '/home/vardan/Desktop/youtubeData2.txt'overwrite into table YouTube_data_table;


select country, vtitle, no_of_views from (
	select country, vtitle, no_of_views, 
	rank() over ( partition by country order by no_of_views desc) as rank 
	from YouTube_data_table ) t where rank < 6;
